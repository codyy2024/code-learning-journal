...
- Break down into steps when nesting - taking applying a link to an image for example,
  - firstly you have the anchor tags `<a></a>`
  - then you have the `href` added in `<a href="example.com"></a>`
  - now you **must** have link text for links - in this case the link *text* is the image, so you have the `img` element nested inside `a` element `<a href="example.com"><img></a>`
  ...
  - as the final step add the `target` attribute of the opening `a` tag, and the `alt` of the `img`
```<!DOCTYPE html>
<html>
  <head>
    <meta .../>
